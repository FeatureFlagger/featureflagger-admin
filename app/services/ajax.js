...
// import config from 'featureflagger-admin/config/environment';
...
const ajaxService = AjaxService.extend({
  ...
  _makeRequest(hash) {
    ...
    const isTokenRequest = isGhostRequest && hash.url.match(/authentication\/(?:token|ghost)/);
    const tokenExpiry = this.get('session.authenticated.expires_at');
    const isTokenExpired = tokenExpiry < (new Date()).getTime();
