...
describe('Integration: Component: ffg-task-button', function() {
  setupComponentTest('ffg-task-button', {
    integration: true
  });

  it('renders', function() {
    // sets button text using positional param
    this.render(hbs`{{ffg-task-button "Test"}}`);
    expect(this.$('button')).to.exist;
    expect(this.$('button')).to.contain('Test');
    expect(this.$('button')).to.have.prop('disabled', false);

    this.render(hbs`{{ffg-task-button class="testing"}}`);
  ...
  it('shows success on success', function(done) {
    ...
    run.later(this, function() {
      expect(this.$('button')).to.have.class('ffg-btn-green');
  ...
  it('assigns specified success class on success', function(done) {
    ...
    this.render(hbs`{{ffg-task-button task=myTask successClass="im-a-success"}}`);

    this.get('myTask').perform();

    run.later(this, function() {
      expect(this.$('button')).to.not.have.class('ffg-btn-green');
  ...
  it('shows failure when task errors', function(done) {
    ...
    run.later(this, function() {
      expect(this.$('button')).to.have.class('ffg-btn-red');
  ...
  it('assigns specified failure class on failure', function(done) {
    ...
    this.render(hbs`{{ffg-task-button task=myTask failureClass="im-a-failure"}}`);

    this.get('myTask').perform();

    run.later(this, function() {
      expect(this.$('button')).to.not.have.class('ffg-btn-red');
